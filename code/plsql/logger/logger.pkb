create or replace package body logger_user.logger
as
...
-- CHANGE LOG
-- Scott 2018-11-15 Added p_log_level to standard log
 ...
 function admin_security_check
  ...
  procedure log(
    ...
    p_extra in clob default null,
    p_params in tab_param default logger.gc_empty_tab_param,
    p_log_level in number default logger.g_debug -- SW 20181115
    )
  is
  begin
    ...
    $if $$no_op $then
      null;
    $else
      if ok_to_log(coalesce(p_log_level, logger.g_debug)) then
        log_internal(
          p_text => p_text,
          p_log_level => coalesce(p_log_level, logger.g_debug),
          p_scope => p_scope,
          p_extra => p_extra,
          p_callstack => dbms_utility.format_call_stack,
          p_params => p_params);
